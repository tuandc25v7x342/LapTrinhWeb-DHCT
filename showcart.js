...
const selectedItem = (evt) => { // function with Event Objects
    ...
    function addEventToAllCartButtons() { // được gọi từ product-list.html
        ...
        shoppingCart.addEventListener('click', function () { // click vào giỏ hàng để xem
    ...
    function showCart() { // được gọi từ showcart.html
        if (localStorage.cartItems == undefined) {
            ...
            if (customerCart[0] == null) {
                ...
                customerCart.forEach(item => {
                    total += Number(item.quantity) * Number(item.price.replace(/[^\d\.]/g, ""));
                    bodyRows += '<tr><td>' + ++no + '</td><td>' + item.id + '</td><td>' + item.name +
                        '</td><td>' + item.quantity + '</td><td>' +
                        formatCurrency(item.price.replace(/[^\d\.]/g, "")) + '</td>' +
                        '<td><a class="ref=" onclick="deleteCart(this)">Delete</a></td></tr>';
                });
            }
